                ...
                <form @submit.prevent="addUser">
                    ...
                    <div class="mb-4">
                        ...
                        >
                            ...
                            <option v-if="currentChucVu === 'admin'" value="user">Người dùng</option>
                            <option v-if="currentChucVu === 'admin'" value="staff">Nhân viên</option>
                            <option v-if="currentChucVu === 'staff'" value="user">Người dùng</option>
                        </select>
                    </div>
                ...
                <div class="space-y-2">
                    ...
                    <!-- <p>
                        ...
                        <span :class="selectedUser.ChucVu === 'admin' ? 'text-blue-500' : 'text-yellow-500'">
                            {{ selectedUser.ChucVu === 'admin' ? 'Quản lý' : 'Người dùng' }}
                        </span>
                    </p> -->
                ...
                <form @submit.prevent="saveEditedUser">
                    ...
                    <div class="mb-4">
                        ...
                        <template v-if="currentChucVu === 'staff'">
                            ...
                            >
                                {{
                                    userToEdit.ChucVu === 'staff'
                                        ? 'Nhân viên'
                                        : userToEdit.ChucVu === 'admin'
                                          ? 'Quản lý'
                                          : 'Người dùng'
                                }}
                            </span>
                        ...
                        <template v-else>
                            <!-- Nếu là admin và không phải là admin đang được chỉnh sửa, cho phép chọn ChucVu -->
                            <select
                                v-model="userToEdit.ChucVu"
                                id="ChucVu"
                                class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
    ...
    methods: {
        ...
        async fetchUsers() {
            ...
            try {
                ...
                const res = await fetch(`${import.meta.env.VITE_API_BACKEND}/api/user/getAllUsers`, {
                    method: 'GET',
                    headers: {
        ...
        async addUser() {
            try {
                const staff = JSON.parse(localStorage.getItem('staff'));
                const staffToken = staff.accessToken;
                const res = await fetch(`${import.meta.env.VITE_API_BACKEND}/api/user/createUserFromAdmin`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        Authorization: `Bearer ${staffToken}`,
        ...
        async saveEditedUser() {
            try {
                ...
                const res = await fetch(`${import.meta.env.VITE_API_BACKEND}/api/user/updateInfoFromAdmin/${userId}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        Authorization: `Bearer ${staffToken}`,
        ...
        async deleteUser(user) {
            ...
            try {
                ...
                const res = await fetch(`${import.meta.env.VITE_API_BACKEND}/api/user/${user._id}`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        Authorization: `Bearer ${staffToken}`,
        ...
        async toggleLockUser(user) {
            try {
                ...
                const res = await fetch(`${import.meta.env.VITE_API_BACKEND}/api/user/locked/${user._id}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        Authorization: `Bearer ${staffToken}`,
